...
包括判断 Path 对应的路径是否存在,获取 Path 对应路径的各种属性(如是否只读、是文件还是文件夹等),甚至可以对文件进行读写。

提示：PurePath 和 Path 最根本的区别在于：PurePath 的本质依然是字符串,
...
Path对象包含了大量 is_xxx() 方法,用于判断该 Path 对应的路径是否为xxx。
...
"""
上面程序中第一行粗体字代码调用了 Path 的 iterdir()方法,该方法将会返回当前目录下的所有文件和子目录;
第二行粗体字代码调用了 glob()方法,获取上一级目录及其所有子目录下的 *.py 文件;
第三行粗体字代码用于获取 g:/publish/codes目录及其所有子目录下的 Path_test1.py文件。

从上面的输出结果来看,不管是遍历当前目录下的文件和子目录,还是搜索指定目录及其子目录,Path 对象都能用一个方法搞定------
