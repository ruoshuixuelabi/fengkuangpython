...
(1)使用二进制模式读取,然后用bytes的 decode()方法恢复成字符串。
(2)利用 codecs 模块的open()函数来打开文件,该函数在打开文件时允许指定字符集。
...
f.close()
"""
上面程序在调用open()函数时,传入了rb模式,这表明采用二进制模式读取文件,此时文件对象的read()方法返回的是bytes对象,
程序可调用bytes对象的 decode()方法将它恢复成字符串。 
