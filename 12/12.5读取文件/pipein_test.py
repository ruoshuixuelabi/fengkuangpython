...
上面语法的作用是：cmd1 命令的输出,将会传给cmd2 命令作为输入;cmd2 命令的输入,又会传给 cmd3 命令作为输出。

下面的 Python 程序用于读取 sys.stdin 的输入,并通过正则表达式识别其中包含多少个E-mail 地址。
"""
import sys
import re
...
上面的管道命令由两个命令组成。
(1)type ad.txt: 该命令使用 type 读取 ad.txt 文件的内容,并将文件内容输出到控制台。但由于使用了管道,因此该命令的输出会传给下一个命令。
