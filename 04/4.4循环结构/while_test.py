...
while循环在每次执行循环体之前,都要先对 test_expression 循环条件求值,如果循环条件为真,则运行循环体部分。
从上面的语法格式来看,迭代语句 iteration_statements 总是位于循环体的最后,因此只有当循环体能成功执行完成时,
while 循环才会执行迭代语句 iteration_statements。
...
print("永远无法跳出的循环体")
"""
在上面代码中,count_i2 的值越来越小,这将导致 count_i2 的值永远小于10,count_i2 <10 循环条件一直为True, 从而导致这个循环永远无法结束。
"""