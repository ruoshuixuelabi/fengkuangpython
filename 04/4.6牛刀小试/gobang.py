"""
4.6.3   控制台五子棋
1.  利用二维列表还可以完成五子棋、连连看、俄罗斯方块、扫雷等常见小游戏。
下面简单介绍利用二维列表实现五子棋。先定义一个二维列表作为棋盘,每当一个棋手下一步棋时,也就是为二维列表的一个数组元素赋值。
下面程序完成了这个游戏的初步功能。
2.  从图4.3来看,程序上面显示的黑点是棋手下的棋,电脑还没有下棋,电脑下棋可以使用随机生成的两个坐标值来控制,
当然也可以增加人工智能(但这已经超出了本书的范围,实际上也很简单)来控制下棋。
3.  读者需要在这个程序的基础上进行完善,保证在用户和电脑下的棋的坐标上不能有棋子(通过判断对应数组元素只能是“十”来确定),
