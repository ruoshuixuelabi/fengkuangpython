"""
1. Python 的整型数值有4种表示形式。
(1)十进制形式：最普通的整数就是十进制形式的整数。
(2)二进制形式：以0b或 0B 开头的整数就是二进制形式的整数。
(3)八进制形式：以0o 或 0O 开头的整数就是八进制形式的整数(第二个字母是大写或小写的O)。
(4)十六进制形式： 以 0x 或 0X 开头的整数就是十六进制形式的整数，其中10~15分别以a~f(此处的a~f不区分大小写)来表示。
2. 为了提高数值(包括浮点型)的可读性， Python 3.x 允许为数值(包括浮点型)增加下画线作为分隔符。
这些下画线并不会影响数值本身。例如如下代码(程序清单同上)。
3. Python 2.x还支持八进制形式的整数(要求以0开头),但由于八进制形式的整数其实很“鸡肋”,
所以Python 3.x删除了这个功能。此外，Python 2.x不支持在数值中使用下画线，
"""
