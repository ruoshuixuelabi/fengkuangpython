"""
2.6.6 比较运算符与bool 类型
1. Python提供了bool类型来表示真(对)或假(错),比如常见的5>3比较算式,这个是正确的,
在程序世界里称之为真(对),Python使用True来代表；再比如4>20比较算式,这个是错误的,
在程序世界里称之为假(错),Python使用False来代表。
2. 由此可见,bool 类型就是用于代表某个事情的真(对)或假(错),如果这个事情是正确的,用True代表；如果这个事情是错误的,用False代表。
3. 比较运算符用于判断两个值(这两个值既可以是变量,也可以是常量,还可以是表达式)之间的大小,
比较运算的结果是bool值(True代表真,False代表假)。Python支持的比较运算符如下。
...
4. Python 的两个bool值分别是True 和 False,但实际上True也可被当成整数1使用，False也可被当成整数0使用。
也就是说， True、False两个值完全可以参与各种算术运算。
5. 关于 == 与 is 看上去很相似，但实质上有区别，== 只比较两个变量的值，但is要求两个变量引用同一个对象。
"""
# 输出True
print("5是否大于 4：", 5 > 4)
# 输出False
