"""
2.6.8 三目运算符
1. Python可通过if语句来实现三目运算符的功能，因此可以近似地把这种if语句当成三目运算符。作为三目运算符的if语句的语法格式如下：
2. True  statements       if       expression       else       False  statements
3. 三目运算符的规则是：先对逻辑表达式expression求值，如果逻辑表达式返回True, 则执行并返回True statements 的值；
如果逻辑表达式返回False, 则执行并返回False statements的值。看如下代码。
4. Python 允许在三目运算符的True statements或 False statements中放置多条语句。 Python主要支持两种放置方式。
