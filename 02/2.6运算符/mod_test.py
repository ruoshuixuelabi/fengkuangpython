...
但5.2 % -2.9的结果有点奇怪，我们预计它为-0.6,但实际输出的是-0.5999999999999996。这里有两个问题。
(1)第一个问题：为什么预计 5.2 % -2.9的结果是-0.6呢?因为Python求余运算的逻辑是用被除数减去除数的N 倍，
此处的N 是-2,因此得到结果是-0.6。
(2)第二个问题：为什么实际输出的是-0.5999999999999996呢?这是由浮点数的存储机制导致的。
计算机底层的浮点数的存储机制并不是精确保存每一个浮点数的值，
读者暂时不需要 花太多的时间去理解浮点数的存储机制，只要知道浮点数在Python 中可能产生精度丢失的问题部行。
