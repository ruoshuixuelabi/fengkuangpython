"""
2.5.3 序列相关方法
1. 字符串本质上就是由多个字符组成的，因此程序允许通过索引来操作字符，比如获取指定索引处的字符，获取指定字符在字符串中的位置等。
2. Python 字符串直接在方括号([])中使用索引即可获取对应的字符，字符串中第一个字符的索引为0、第二个字符的索引为1,后面各字符依此类推。
...
# 如果要获取字符串的长度，则可调用Python 内置的len()函数
# 输出s的长度
print(len(s))  # 24
# 输出'test'的长度
print(len('test'))  # 4
