"""
2.5.6  查找、替换相关方法
1. str还提供了如下常用的执行查找、替换等操作的方法。
(1)startswith(): 判断字符串是否以指定子串开头。
(2)endswith():  判断字符串是否以指定子串结尾。
(3)find():   查找指定子串在字符串中出现的位置，如果没有找到指定子串，则返回-1。
(4)index(): 查找指定子串在字符串中出现的位置，如果没有找到指定子串，则引发ValueError 错误。
(5)replace():  使用指定子串替换字符串中的目标子串。
(6)translate(): 使用指定的翻译映射表对字符串执行替换。
2. 从下面程序可以看出，str的 translate()方法需要根据翻译映射表对字符串进行查找、替换。
在下面程序中我们自己定义了一个翻译映射表，这种方式需要开发者能记住所有字符的编码，这显然不太可能。
为此， Python 为 str类提供了一个maketrans()方法，通过该方法可以非常方便地创建翻译映射表。
3. 假如定义a->α、b->β、t->t的映射，程序只要将需要映射的所有字符作为maketrans()方法的第一个参数，
将所有映射的目标字符作为 maketrans()方法的第二个参数即可。
table =  str.maketrans('abt','αβt')
4. 从执行过程可以看到，不管是自己定义的翻译映射表，还是使用 maketrans()方法创建的翻译映射表，
其实都是为了定义字符与字符之间的对应关系，只不过该翻译映射表不能直接使用字符本身，必须使用字符的编码而已。
5. 需要指出的是，如果使用 Python 2.x,str类并没有 maketrans()方法，而是由 string 模块提供 maketrans()函数，
因此程序需要先导入 string 模块，然后调用该模块的 maketrans()函数
6. 但 Python 2.x中的翻译映射表不如Python 3.x的翻译映射表直观、明了，
