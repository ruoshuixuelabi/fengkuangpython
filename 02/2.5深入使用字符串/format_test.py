"""
2.5.2 字符串格式化
1. Python 提供了"%"对各种类型的数据进行格式化输出，例如如下代码。
2. 格式化字符串中的"%s"被称为转换说明符(Conversion Specifier), 其作用相当于一个占位符，它会被后面的变量或表达式的值代替。
"%s"指定将变量或值使用str()函数转换为字符串。
3. 如果格式化字符串中包含多个"%s"占位符，第三部分也应该对应地提供多个变量，并且使用圆括号将这些变量括起来。例如如下代码。
4. 在格式化字符串中难道只能使用"%s"吗?还有其他转换说明符吗?如果只有"%s"这一种形式， Python 的格式化功能未免也太单一了。
...
$	    使用str()将变量或表达式转换为字符串
5. 在默认情况下，转换出来的字符串总是右对齐的，不够宽度时左边补充空格。
Python 也允许在最小宽度之前添加一个标志来改变这种行为， Python 支持如下标志。
(1) - :   指定左对齐
(2) +:   表示数值总要带着符号(正数带"+",负数带"-")。
(3) 0:   表示不补充空格，而是补充0。
这三个标志可以同时存在
6. 对于转换浮点数，Python 还允许指定小数点后的数字位数；
如果转换的是字符串， Python 允许指定转换后的字符串的最大字符数。
