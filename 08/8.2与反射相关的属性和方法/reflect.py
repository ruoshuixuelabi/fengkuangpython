"""
如果程序在运行过程中要动态判断是否包含某个属性(包括方法),甚至要动态设置某个属性值,则可通过 Python 的反射支持来实现。
...
print(c.view_times)
"""
上面程序先定义了一个 Comment 类,接下来程序创建了 Comment 类的实例。
程序后面部分示范了 hasattr()、getattr()、setattr()三个函数的用法。从上面的示例代码可以看出,
hasattr()函数既可判断属性,也可判断方法,但 getattr()则只能获取属性的属性值。
...
从上面最后两行输出来看,程序使用 setattr() 函数可改变 Python 对象的属性值：如果使用该函数对 Python 对象设置的属性不存在,
那么就表现为添加属性——反正Python 是动态语言。看如下代码。
"""
# 设置不存在的属性,即为对象添加属性
...
上面程序先定义了一个 bar()函数,在该函数中不能定义 self 参数(否则需要程序员显式为参数传入参数值,
系统不会自动为该参数绑定参数值)。接下来程序调用 setattr() 函数将 Comment  对象的 info() 方法设置为 bar()函数,
然后程序调用 Comment 对象的info()方法,其实就是调用程序中的 bar()函数。
...
c.info()
"""
上面第一行代码将 c 的 info(原本是方法)设置为字符串'fkit',这样 info 就变成了属性,而不是方法。
运行上面第二行代码,将会看到如下错误提示。
