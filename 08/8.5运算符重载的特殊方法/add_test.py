"""
8.5	运算符重载的特殊方法
1.  Python允许为自定义类提供特殊方法,这样就可以让自定义类的对象也支持各种运算符的运算。

8.5.1	与数值运算符相关的特殊方法

2.  根据第2章的介绍,与数值运算相关的运算符包括算术运算符、位运算符等,其实这些运算符都是由对应的方法提供支持的。
开发人员可以为自定义类提供如下方法。
...
3.  提示：本书暂不介绍Python 的矩阵运算支持,如果读者对Python 的矩阵运算感兴趣,
则需要先为Python 安装numpy模块,通过命令行窗口输入pip3 install numpy命令来安装 numpy 模块;
然后再导入numpy 模块,可以通过在程序中添加from numpy import *或 I 	import numpy as np语句来导入numpy 模块。
4.  一旦为自定义类提供了上面这些方法,程序就可以直接用运算符来操作该类的实例。比如程序执行 x +y,
相当于调用x.__add__(self,y),因此只要x所属的类提供__add__(self,other)方法即可;
如果自定义类没有提供对应的方法,程序会返回Notlmplemented。
5.  例如,下面程序定义了一个Rectangle类,如果希望对两个Rectangle执行加法运算,则可为该类提供 __add__(self,other)方法。
6.  上面程序为Rectangle提供了 __add__ 方法,因此程序就可以对两个Rectangle使用"+"执行加法运算了。
运行上面程序,可以看到如下输出结果。
7.  当程序执行x+y 运算时, Python 首先会尝试使用x的 __add__ 方法进行计算;如果x没有提供 __add__ 方法,
...
8.  简单来说,如果自定义类提供了上面列出的 __rxxx__ ()方法,那么该自定义类的对象就可以出现在对应运算符的右边。
"""
