"""
8.5.3 与单目运算符相关的特殊方法
1.  Python 还提供了+(单目求正)、- (单目求负)、~(单目取反)等运算符,这些运算符也有对应的特殊方法。
(1)object.__neg__(self): 为单目求负(-)运算符提供支持。
(2)object.__pos__(self):为单目求正(+)运算符提供支持。
(3)object.__invert__(self):为单目取反(~)运算符提供支持。
2.  下面程序为Rectangle类实现了一个 __neg__()方法,该方法用于控制将矩形的宽、高交换。下面是该类的代码。
3.  上面程序中粗体字代码定义了 __neg__方法,这样即可对 Rectangle 对象执行求负运算,在  __neg__ 方法内部就是交换矩形的宽和高,
