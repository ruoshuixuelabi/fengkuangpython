...
Python提供了 str()、int()、float()、complex()等函数(其实是这些类的构造器)将其他类型的对象转换成
...
提示：对象的 __str__()和 __repr__()方法的功能有些类似,它们都用于将对象转换成字符串,区别在于：
__repr__代表的是"自我描述"的方法,当程序调用print()函数输出对象时,Python 会自动调用该对象的 __repr__()方法,
而 __str__()方法则只有在显式调用str()函数时才会起作用。

下面还是以自定义的 Rectangle 为例,程序为该类提供了一个 __int__() 方法,这样程序就可用 int()函数将Rectangle对象转换成整数了。
"""
...
print(int(r))  # 20
"""
上面程序中粗体字代码实现了 __int__ 方法,该方法返回 Rectangle 对象的面积转换成的整数,
因此程序可调用int()函数将Rectangle转换成整数——实际上就是返回该矩形的面积。
