"""
1.  下面程序使用 Rectangle类,并为该类定义了一个 __radd__ 方法,这样即使运算符左边的对象没有提供对应的运算符方法,
但是只要把Rectangle对象放在运算符的右边,程序也一样可以执行运算。
2.  此外, Python 还支持各种扩展后的赋值运算符,这些扩展后的赋值运算符也是由特殊方法来提供支持的。
...
(8)object.__idivmod__(self,other): 求余运算,为divmod 运算符提供支持。
