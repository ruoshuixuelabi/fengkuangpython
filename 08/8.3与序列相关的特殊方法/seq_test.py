...
Python 的序列可包含多个元素,开发者只要实现符合序列要求的特殊方法,就可实现自己的序列。

8.3.1 序列相关方法

序列最重要的特征就是可包含多个元素,因此和序列有关的特殊方法有如下几个。
(1)__len__(self)：该方法的返回值决定序列中元素的个数。
(2)__getitem__(self,key)：该方法获取指定索引对应的元素。该方法的 key 应该是整数值或 slice 对象,否则该方法会引发 KeyError 异常。
(3)__contains__(self,item)：该方法判断序列是否包含指定元素。
(4)__setitem__(self,key,value)：该方法设置指定索引对应的元素。该方法的 key 应该是整数值或 slice 对象,否则该方法会引发 KeyError 异常。
...
下面程序将会实现一个字符串序列,在该字符串序列中默认每个字符串的长度都是3,该序列的元素按AAA、AAB、AAC……这种格式排列。
...
print(sq[1])  # crazyit
"""
上面程序实现了一个 StringSeq 类,并为该类实现了 __len__()、__getitem__()、__setitem__()和 __delitem__()方法,
其中 __len__()方法返回该序列包含的元素个数,__getitem__()方法根据索引返回元素,__setitem__()方法根据索引修改元素的值,
而和 __delitem__()方法则用于根据索引删除元素。

该序列本身并不保存序列元素,序列会根据索引动态计算序列元素,因此该序列需要保存被修改、被删除的元素。
该序列使用 __changed 实例变量保存被修改的元素,使用 __changed 实例变量(列表)保存被删除的索引。
...
从上面的输出结果来看,程序中序列的第二个元素sq[1]恰好为'AAB',程序既可对序列元素赋值,
