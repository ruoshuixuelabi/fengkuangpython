...
(对于 Python 而言,方法相当于属性值为函数的属性)。例如如下程序。
"""
...
print(gd.speed)
"""
上面程序中第一行粗体字代码定义了 __slots__ = ('walk', 'age', 'name'), 这意味着程序只允许为 Dog 实例动态添加
walk、age、name 这三个属性或方法。因此上面程序中第二行、第三行粗体字代码为 Dog 对象动态添加walk()方法和 age 属性都是允许的。

但如果程序尝试为Dog 对象添加其他额外属性,程序就会引发AttributeError错误,如下面最后一行代码所示。
运行上面程序,可以看到如下输出结果。
Traceback (most recent call last):
  File "slots_test.py", line 24, in <module>
    d.foo = 30 # AttributeError
...
上面代码通过Dog 类动态添加了bar()方法,这样 Dog 对象就可以调用该bar()方法了。

此外,__slots__ 属性指定的限制只对当前类的实例起作用,对该类派生出来的子类是不起作用的。例如如下代码(程序清单同上)。
...
如果要限制子类的实例动态添加属性和方法,则需要在子类中也定义 __slots__ 属性,这样,
