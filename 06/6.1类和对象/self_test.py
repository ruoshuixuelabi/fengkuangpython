"""
1.  需要说明的是,自动绑定的self参数并不依赖具体的调用方式,不管是以方法调用还是以函数调用的方式执行它,self参数一样可以自动绑定。
例如如下程序。
2.  下面程序中第一行粗体字代码以方法形式调用User对象的test()方法,此时方法调用者当然会自动绑定到方法的第一个参数(self参数);
程序中第二行粗体字代码以函数形式调用User对象的test()方法,看上去此时没有调用者了,
