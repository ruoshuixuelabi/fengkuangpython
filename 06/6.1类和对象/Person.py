"""
在设计之初,Python 就被设计成支持面向对象的编程语言,因此 Python 完全能以面向对象的方式编程。
而且 Python 的面向对象比较简单,它不像其他面向对象语言提供了大量繁杂的面向对象特征,它致力于提供简单、够用的语法功能。

正因为如此,在 Python 中创建一个类和对象都很容易。Python 支持面向对象的三大特征：封装、继承和多态,
...
如果不为类定义任何类变量和方法,那么这个类就相当于一个空类,如果空类不需要其他可执行语句,则可使用 pass 语句作为占位符。
class  Empty
    pass
...
程序也可在任何地方为已有的类增加变量;程序可通过 del 语句删除已有类的类变量。

类似的是,Python 对象的实例变量也可以动态增加或删除——只要对新实例变量赋值就是增加实例变量,
因此程序可以在任何地方为已有的对象增加实例变量;程序可通过 del 语句删除已有对象的实例变量。

在类中定义的方法默认是实例方法,定义实例方法的方法与定义函数的方法基本相同,
只是实例方法的第一个参数会被绑定到方法的调用者(该类的实例)——因此实例方法至少应该定义一个参数,该参数通常会被命名为 self。
...
如果开发者没有为该类定义任何构造方法,那么 Python 会自动为该类定义一个只包含一个 self 参数的默认的构造方法。
...
class Person:
    ...
    def __init__(self, name='Charlie', age=8):
        # 这两行代码用传入的name、age 参数(这两个参数都有默认值)对 self 的 name、age 变量赋值。
        # 由于 Python 的第一个self参数是自动绑定的(在构造方法中自动绑定到该构造方法初始化的对象),
...
如下代码示范了调用 Person 类的构造方法(程序清单同上)。
"""
...
p.name = '李刚'
# 调用p的say()方法,声明say()方法时定义了2个形参,但第一个形参(self)是自动绑定的,因此调用该方法只需为第二个形参指定一个值
...
这两行代码用传入的 name、age 参数(这两个参数都有默认值)对 self 的 name、age 变量赋值。
由于 Python 的第一个 self 参数是自动绑定的(在构造方法中自动绑定到该构造方法初始化的对象),
而这两行代码就是对 self 的 name、age 两个变量赋值,也就是对该构造方法初始化的对象(p 对象)的 name、age 变量赋值,
即为 p 对象增加了 name、age 两个实例变量。

上面代码中通过 Person 对象调用了 say()方法,在调用方法时必须为方法的形参赋值。
但 say()方法的第一个形参 self 是自动绑定的,它会被绑定到方法的调用者(p),
因此程序只需要为 say()方法传入一个字符串作为参数值,这个字符串将被传给 content参数。

大部分时候,定义一个类就是为了重复创建该类的对象,同一个类的多个对象具有相同的特征,而类则定义了多个对象的共同特征。
从某个角度来看,类定义的是多个对象的特征,因此类不是一个具体存在的实体,对象才是一个具体存在的实体。
完全可以这样说：你不是人这个类,我也不是人这个类,我们都只是人的对象。

6.1.3 对象的产生和使用

由于 Python 是动态语言,因此程序完全可以为 p 对象动态增加实例变量——只要为它的新变量赋值即可;
也可以动态删除实例变量——使用 del 语句即可删除。例如如下代码(程序清单同上)。
"""
# 为p对象增加一个skills实例变量
...
接下来程序中的粗体字代码调用 del 删除了 p 对象的 name 实例变量,当程序再次访问 print(p.name)时就会导致 AttributeError 错误,
并提示：AttributeError: 'Person' object has no attribute 'name'

Python 是动态语言,当然也允许为对象动态增加方法。
比如上面程序中在定义 Person 类时只定义了一个 say()方法,但程序完全可以为 p 对象动态增加方法。
但需要说明的是,为 p 对象动态增加的方法,Python 不会自动将调用者自动绑定到第一个参数(即使将第一个参数命名为 self 也没用)。
例如如下代码(程序清单同上)。
"""
...
p.intro("生活在别处")
"""
正如从上面粗体字代码所看到的,通过 MethodType 包装 intro_func 函数之后(包装时指定了将该函数的第一个参数绑定为 p),
为 p 对象动态增加的 intro() 方法的第一个参数已经绑定,因此程序通过 p 调用 intro()方法时无须传入第一个参数——
就像定义类时已经定义了 intro()方法一样。
"""
