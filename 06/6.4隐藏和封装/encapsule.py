...
Python并没有提供类似于其他语言的private等修饰符,因此 Python 并不能真正支持隐藏。

为了隐藏类中的成员,Python 玩了一个小技巧：只要将 Python 类的成员命名为以双下画线开头的,Python 就会把它们隐藏起来。
...
# 尝试调用隐藏的__hide()方法 AttributeError: 'User' object has no attribute '__hide'
# u.__hide()
# Python 其实没有真正的隐藏机制,双下画线只是Python 的一个小技巧：
# Python 会"偷偷"地改变以双下画线开头的方法名,会在这些方法名前添加单下画线和类名。
# 因此上面的 __hide()方法其实可以按如下方式调用(通常并不推荐这么干)。
# 调用隐藏的__hide()方法
u._User__hide()
# 对隐藏的__name属性赋值
u._User__name = 'fk'
...
print(u.name)
"""
上面程序将 User 的两个实例变量分别命名为 __name 和 __age,这两个实例变量就会被隐藏起来,
...
上面程序用到了raise关键字来抛出异常。关于raise 关键字和异常的相关信息,请参考本书第7章。

从该程序可以看出封装的好处,程序可以将User对象的实现细节隐藏起来,程序只能通过暴露出来的setname()、setage()方法
来改变User 对象的状态,而这两个方法可以添加自己的逻辑控制,这种控制对User 的修改始终是安全的。

最后需要说明的是,Python 其实没有真正的隐藏机制,双下画线只是Python的一个小技巧：
Python 会"偷偷"地改变以双下画线开头的方法名,会在这些方法名前添加单下画线和类名。

类似的是,程序也可通过为隐藏的实例变量添加下画线和类名的方式来访问或修改对象的实例变量。

总结：Python 并没有提供真正的隐藏机制,所以 Python 类定义的所有成员默认都是公开的;如果程序希望将 Python 类中
