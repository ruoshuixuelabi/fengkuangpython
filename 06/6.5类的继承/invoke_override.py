...
如果在子类中调用重写之后的方法,Python 总是会执行子类重写的方法,不会执行父类中被重写的方法。
如果需要在子类中调用父类中被重写的实例方法,那该怎么办呢?

读者别忘了,Python 类相当于类空间,因此 Python 类中的方法本质上相当于类空间内的函数。
所以,即使是实例方法,Python 也允许通过类名调用。区别在于：在通过类名调用实例方法时,
Python 不会为实例方法的第一个参数 self 自动绑定参数值,而是需要程序显式绑定第一个参数self。 这种机制被称为未绑定方法。
...
sc.bar()
"""
上面程序中 SubClass 继承了 BaseClass 类,并重写了父类的 foo()方法。
