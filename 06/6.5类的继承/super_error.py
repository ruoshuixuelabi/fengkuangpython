...
m.info()  # ②
"""
上面程序中粗体字代码定义了 Manager 类,该类继承了 Employee 和 Customer 两个父类。
...
但是当执行到②号代码时就会引发错误,这是由于程序在使用 Employee 类的构造方法创建 Manager 对象时,
程序并未初始化 Customer 对象所需的两个实例变量：favorite 和 address,因此程序引发错误。
...
上面代码为 Manager 的构造方法传入两个参数,这明显是调用从 Customer 类继承得到的两个构造方法,
此时程序将可以初始化 Customer 类中的 favorite 和 address 实例变量,但它又不能初始化 Employee 类中的 salary 实例变量。
因此,此时程序中的②号代码可以正常运行,但①号代码会报错。

为了让 Manager 能同时初始化两个父类中的实例变量,Manager 应该定义自己的构造方法——就是重写父类的构造方法。
...
从上面介绍可以看出, super其实是一个类,因此调用 super() 的本质就是调用super 类的构造方法来创建super对象。

从上面的帮助信息可以看到,使用super()构造方法最常用的做法就是不传入任何参数(这种做法与super(type,obj)的效果相同),
