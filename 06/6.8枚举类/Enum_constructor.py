...
1.  枚举也是类,因此枚举也可以定义构造器。为枚举定义构造器之后,在定义枚举实例时必须为构造器参数设置值。例如如下程序。
2.  下面程序定义了 Gender 枚举类,并为它定义了一个构造器,调用该构造器需要传入 cn_name 和desc两个参数,
因此程序使用如下代码来定义 Gender 的枚举值。
3.  下面代码为 MALE  枚举指定的value是'男'和'阳刚之力'这两个字符串,其实它们会被自动封装成元组后传给 MALE  的 value 属性；
