"""
前面介绍的绝大部分程序,都只是在做单线程的编程。前面所有程序(除第11章中的程序之外,
它们有内建的多线程支持)都只有一条顺序执行流——程序依次向下执行每行代码,如果程序在执行某行代码时遇到阻塞,则程序将会停滞在该处。
使用IDE工具的单步调试功能,就可以非常清楚地看出这一点。

但实际的情况是,单线程的程序往往功能非常有限。例如,开发一个简单的服务器程序,当这个服务器程序需要向不同的客户端提供服务时,
不同的客户端之间应该互不干扰。多线程听上去是非常专业的概念,其实它非常简单——单线程的程序(前面介绍的绝大部分程序)
只有一个顺序执行流,而多线程的程序则可以包含多个顺序执行流,这些顺序执行流之间互不干扰。
可以这样理解： 单线程的程序如同只雇佣一个服务员的餐厅,他必须做完一件事情后才可以做下一件事情;
而多线程的程序则如同雇佣多个服务员的餐厅,他们可以同时做多件事情。

Python 语言提供了非常优秀的多线程支持,程序可以通过非常简单的方式来启动多线程。
本章将会详细介绍 Python 多线程编程的相关知识,包括创建、启动线程,控制线程,以及多线程的同步操作,
并会介绍如何利用 Python 内建支持的线程池来提高多线程的性能。
