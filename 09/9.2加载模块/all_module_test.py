"""
1.  上面的 __all__ 变量指定该模块默认只被导入hello和world 两个程序单元。下面程序示范了模块中  __all__ 变量的用处。
2.  上面第一行粗体字代码使用"from all_module import *"导入了 all_module 模块下所有的程序单元。
由于该模块包含了 __all__ 变量,因此该语句只导入 __all__ 变量所列出的程序单元。
3.  从上面的输出结果可以看到,通过"from all_module import *"语句确实只能导入 __all__ 变量所列出的全部程序单元,
没有列出的test就没有被导入进来。
4.  事实上, __all__ 变量的意义在于为模块定义了一个开放的公共接口。通常来说,只有 __all__ 变量列出的程序单元,
才是希望该模块被外界使用的程序单元。因此,为模块设置  __all__ 变量还是比较有用的。
比如一个实际的大模块可能包含了大量其他程序不需要使用的变量、函数和类,那么通过 __all__ 变量即可把它们自动过滤掉,这还是非常酷的。
5.  如果确实希望程序使用模块内  __all__ 列表之外的程序单元,有两种解决方法。
