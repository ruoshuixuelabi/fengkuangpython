"""
1.  在使用from…import语法时也可一次导入指定模块内的所有成员,例如如下程序。
2.  上面粗体字代码一次导入了sys模块中的所有成员,这样程序即可通过成员名来使用该模块内的所有成员。
该程序的输出结果和前面程序的输出结果完全相同。
3.  需要说明的是, 一般不推荐使用“from 模 块 import*”这种语法导入指定模块内的所有成员, 因为它存在潜在的风险。
