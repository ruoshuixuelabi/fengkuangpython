...
上面三个模块文件都位于 fk_package包下的包下,总共提供了两个函数和一个类。
这意味着： fk_package 包(也是模块)总共包含 arithmetic_chart、billing和 print_shape 三个模块。
在这种情况下,这三个模块就相当于 fk_package 包的成员。

9.3.3 导入包内成员

如果需要使用 arithmetic_chart、billing和 print_shape这三个模块,则可以在程序中执行如下导入代码
"""
...
"""
上面程序中第一行粗体字代码是"import fk_package",由于导入包的本质只是加载并执行包里的 __init__.py 文件,
因此执行这条导入语句之后,程序只能使用 fk_package 目录下的 __init__.py 文件中定义的程序单元。
对于本例而言,由于fk_package\__init__.py文件内容为空,因此这条导入语句没有任何作用。

第二行粗体字代码是"import fk_package.print_shape",这条导入语句的本质就是加载并执行 fk_package 包下的 print_shape.py文件,
并将其赋值给 fk_package.print_shape 变量。因此执行这条导入语句之后,程序可访问
fk_package\print_shape.py 文件所定义的程序单元,但需要添加 fk_package.print_shape 前缀。

第三行粗体字代码是"from fk_package import billing",
这条导入语句的本质是导入 fk_package 包(也是模块)下的billing成员(其实是模块)。
因此执行这条导入语句之后,程序可使用 fk_package\billing.py文件定义的程序单元,而且只需要添加billing前缀。

第四行粗体字代码与第二行粗体字代码的导入效果相同。

该程序后面分别测试了 fk_package 包下的 print_shape、billing、arithmetic_chart这三个模块的功能。
...
想一想就知道：包内的 __init__.py 文件并不是用来定义程序单元的,而是用于导入该包内模块的成员,
