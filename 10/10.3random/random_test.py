...
在 Python 的交互式解释器中先导入 random 模块,然后输入random.__all__ 命令( __all__ 变量代表了该模块开放的公开接口),
即可看到该模块所包含的全部属性和函数。
import random
random.__all__
['Random', 'SystemRandom', 'betavariate', 'choice', 'choices', 'expovariate', 'gammavariate', 'gauss', 'getrandbits',
'getstate', 'lognormvariate', 'normalvariate', 'paretovariate', 'randbytes', 'randint', 'random', 'randrange', 'sample',
...
(2)random.randrange(start,stop[,step])：返回从 start 开始到 stop 结束、步长为 step 的随机数。
...
提示：关于生成伪随机浮点数的函数,Python 还提供了random.triangular(low,high, mode)、random.gauss(mu,sigma)等,
它们用于生成呈对称分布、高斯分布的随机数。由于这种随机数需要一些数学知识来理解它们,而且平时开发并不常用,
因此这里就不深入展开介绍了。

下面程序示范了random 模块中常见函数的功能和用法。
"""
import random
