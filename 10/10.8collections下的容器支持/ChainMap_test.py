"""
在 collections 包中除包含 deque 容器类之外,还包含另外一些容器类,这些容器类可能不如前面介绍的容器类那么常用,
...
简单来说,ChainMap 相当于把多个 dict 合并成一个大的 dict,但实际上底层并未真正合并这些 dict,
因此程序无须调用多个update()方法将多个dict进行合并。所以说ChainMap 是一种"假"合并,但实际用起来又具有较好的效果。

需要说明的是,由于 ChainMap 只是将多个dict链在一起,并未真正合并它们,因此在多个 dict 中完全可能具有重复的key,
在这种情况下,排在"链"前面的dict中的key 具有更高的优先级。

例如,下面程序示范了ChainMap 的用法。
"""
...
print(cm['Go'])  # 93
"""
上面程序中粗体字代码将a、b、c 三个 dict 链在一起,组成一个 ChainMap,这个链的顺序就是 a、b、c,
...
从上面第一行输出可以看到,ChainMap 其实并未将这三个 dict 合并成一个大的 dict,只是将它们链在一起而已。
"""
