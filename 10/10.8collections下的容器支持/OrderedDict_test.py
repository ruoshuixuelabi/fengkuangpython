...
OrderedDict 也是 dict 的子类,其最大特征是：它可以"维护"添加key-value对的顺序。
...
for k, v in d.items():
    print(k, v)
"""
上面程序首先创建了 OrderedDict 对象,接下来程序向其中添加了4个 key-value 对,OrderedDict 完全可以"记住"它们的添加顺序。
运行该程序,可以看到如下输出结果。
...
正如前面所说的,两个 OrderedDict 中即使包含的 key-value 对完全相同,但只要它们的顺序不同,
...
print(d1 == d2)  # False
"""
上面程序先创建了一个普通的 dict 对象,该对象中包含4个key-value对;
