...
1.  collections包下的 Counter 也是一个很有用的工具类,它可以自动统计容器中各元素出现的次数。
2.  Counter 的本质就是一个特殊的 dict,只不过它的 key 都是其所包含的元素,而它的value则记录了该 key 出现的次数。
因此,如果通过 Counter 并不存在的 key 访问 value,将会输出0------代表该 key 出现了0次。
3.  程序可通过任何可迭代对象参数来创建 Counter 对象,此时 Counter将会自动统计各元素出现的次数,并以元素为key,
