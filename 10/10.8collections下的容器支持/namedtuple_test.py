...
1.  namedtuple()是一个工厂函数,使用该函数可以创建一个 tuple 类的子类,该子类可以为 tuple 的每个元素都指定字段名,
这样程序就可以根据字段名来访问 namedtuple 的各元素了。当然,如果有需要,程序依然可以根据索引来访问 namedtuple 的各元素。
2.  namedtuple 是轻量级的,性能很好,其并不比普通 tuple 需要更多的内存。
3.  namedtuple 函数的语法格式如下。
...
5.  下面程序示范了使用namedtuple工厂函数来创建命名元组。
6.  除上面介绍的用法之外,Python 还为命名元组提供了如下方法和属性。
...
"""
1.  上面程序先创建了一个命名元组类：Point,它是 tuple 的子类,因此程序既可使用命名参数的方式设置元组成员,也可使用索引的方式设置元组成员。
2.  在创建了 Point 对象之后,Point 就代表一个命名元组,程序既可使用普通元组的方式来访问元组的成员,也可使用字段名来访问元组的成员。
"""
