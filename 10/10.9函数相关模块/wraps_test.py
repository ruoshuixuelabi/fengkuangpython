"""
@wraps(wrapped_func)函数装饰器与 update_wrapper(wrapper, wrapped_func)函数的作用是一样的,
...
print(test.__doc__)
"""
上面程序中粗体字代码的作用是：让被包装函数(wrapper)就像 f 函数。

上面程序使用 @fk_decorator 修饰 test() 函数,因此在调用 test()函数时,实际上是调用 fk_decorator 的返回值：
wrapper 函数—— 这是前面的函数装饰器的功能。

也就是说,上面程序中最后三行代码看上去是访问test函数,实际上是访问 wrapper 函数。
...
如果注释掉程序中的粗体字代码@wraps(f),此时将不能让 wrapper 函数看上去像test函数。如果再次运行该程序,将看到如下输出结果。
