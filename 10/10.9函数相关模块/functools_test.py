"""
1.  在 functools 模块中主要包含了一些函数装饰器和便捷的功能函数。在Python 的交互式解释器中先导入functools模块,
然后输入[e fore in dir(functools) if not e.startswith('_')]命令,即可看到该模 块所包含的全部属性和函数。
2.  在 functools模块中常用的函数装饰器和功能函数如下。
...
其中maxsize 参数用于设置缓存占用的最大字节数, typed 参数用于设置将不同类型的缓存结果分开存放。
...
(4)functools.partial(func,*args,**keywords): 该函数用于为func 函数的部分参数指定参数值,
...
该函数装饰器用于修饰包装函数,使包装函数看上去就像wrapped 函数。
...
下面程序示范了functiontools模块中部分函数或函数装饰器的用法。
"""
...
print(int('10010', 2))
"""
1.  上面程序中第一行粗体字代码调用reduce()函数来计算序列的"累计"结果,在调用该函数时传入的第一个参数(函数)决定了累计算法,
此处使用的累计算法是"累加"。
2.  程序中第二行粗体字代码调用cmp_to_key()函数将老式的大小比较函数(old_cmp)转换为关键字函数,
这样该关键字函数即可作为列表对象的sort()方法的参数。
3.  程序中第三行粗体字代码调用@lru_cache 对函数结果进行缓存,后面程序第一次执行 factorial(5)时将会看到执行结果;
但接下来调用factorial(3)、factorial(5)时都不会看到执行结果,因为它们的结果已被缓存起来。
4.  程序中第四行粗体字代码调用partial()函数为int()函数的base参数绑定值"2",
