...
1.  Python 完全支持函数式编程,它还提供了一些与函数相关的模块。

10.9.1	itertools模块的功能函数

2.  在 itertools 模块中主要包含了一些用于生成迭代器的函数。在Python 的交互式解释器中先导入 itertools 模块,
然后输入[e for e in dir(itertools) if not e.startswith('_')]命令,即可看到该模块所包含的全部属性和函数。
3.  从上面的输出结果可以看出, itertools 模块中的不少函数都可以用于生成迭代器。 先看 itertools模块中三个生成无限迭代器的函数。
