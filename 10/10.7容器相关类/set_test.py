"""
除前面介绍的列表(list)、元组(tuple)和字典(dict)等容器类型之外,Python 还提供了集合(set)、双端队列(deque)等数据类型,
这些数据类型可能不如列表、字典等容器类型常用,但它们也是 Python 编程的基础内容,也应该重点掌握。

绝大部分编程语言都会提供list、set、dict(在有的语言中叫dictionary或 map)、deque 这些数据类型,
如果你有Java、Objective-C等其他语言的编程经验,那么你会对这些类型感到似曾相识。 这是为什么呢?

早期学计算机编程的人可能听过一句话：程序 = 数据结构 + 算法

一直以来,这句话被国内多少计算机专业奉为圭臬,"数据结构"也是不少本科院校软件专业的基础课程,
"数据结构"课程所讲授的其实就是list、set、dict、deque等内容。其原因就在于：list.  set、dict、deque等数据结构确实是软件开发的基础。

提示：以笔者的经验来看,软件专业一开始就上"数据结构"课程未必是好事,这些内容虽然非常重要,但对于普通开发者而言,
他们往往并不需要掌握实现 list、set、dict、deque 的具体细节,只要会用它们即可。此外,一开始就学习"数据结构"比较容易打
击初学者的信心,因此完全可以等学习者有一定的编程基础之后再来学习这些数据结构

总的来说,绝大部分编程语言通常总会提供的4种主流的数据结构是list、set、dict 和 deque, 其中set集合类似于一个罐子,
把一个对象添加到 set 集合时,set 集合无法记住添加这个元素的顺序,所以 set 里的元素不能重复(否则系统无法准确识别这个元素);
list 容器就是前面介绍的列表,它可以记住每次添加元素的顺序,因此程序可通过索引来存取元素,list 容器的元素允许重复;
dict 容器也像一个罐子,只是它里面的每项数据都由 key-value 对组成,因此程序可通过 key 来存取 value。
...
set集合是可变容器,程序可以改变容器中的元素。与set对应的还有frozenset集合,frozenset 是set的不可变版本,它的元素是不可变的。

在交互式解释器中输入[e for e in dir(set) if not e.startswith('_')]来查看set集合的全部方法,可以看到如下输出结果。
...
对于上面这些方法,其方法名已经暗示了它们的作用,比如add()很明显就是向set集合中添加元素,remove()、discard()就是删除元素,
clear()就是清空set集合,等等。

remove()与 discard()方法都用于删除集合中的元素,但区别在于：如果集合中不包含被删除的元素,
remove()方法会报出 KeyError 异常,而discard()方法则什么也不做。
...
print('e集合的元素：', e)
"""
上面程序基本示范了 set 集合中所有方法的用法。不仅如此,该程序还示范了 set 集合支持的 如下几个运算符。
(1)<=：相当于调用 issubset()方法,判断前面的 set 集合是否为后面的 set 集合的子集合。
(2)>=：相当于调用 issuperset()方法,判断前面的 set 集合是否为后面的 set 集合的父集合。
(3)-：相当于调用 difference()方法,用前面的 set 集合减去后面的 set 集合的元素。 
(4)&：相当于调用 intersection()方法,用于获取两个 set 集合的交集。
(5)^：计算两个集合异或的结果,就是用两个集合的并集减去交集的元素。

此外,由于 set集合本身是可变的,因此它除了提供add()、remove()、discard()方法来操作单个元素,
