"""
re 模块中的 Match 对象(其具体类型为 _sre.SRE_Match)则是 match()、search()方法的返回值,
...
上面这些方法都涉及了组的概念,组是正则表达式中很常见的一个东西：用圆括号将多个表达式括起来形成组。
...
print(m.groups())
"""
上面程序中 search() 函数使用了一个正则表达式：r'(fkit).(org)',在该正则表达式内包含两个组,即(fkit)和(org),
...
从该程序可以看出,只要正则表达式能匹配得到结果,则不管正则表达式是否包含组,group(0)、span(0) 总能获得内容,
因为它们分别是获取整个正则表达式所匹配的子串,以及该子串的开始位置和结束位置。

如果在正则表达式中为组指定了名字(用?P<名字>为正则表达式的组指定名字),
就可以调用groupdict()方法来获取所有组所匹配的字符串组成的字典——其中组名作为字典的 key。 例如如下代码。
"""
