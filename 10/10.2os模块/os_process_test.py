"""
此外,在 os 模块下还包含大量操作文件和目录的功能函数,本书将会在第12章专门介绍这些功能函数。

在 os 模块下还包含各种进程管理函数,它们可用于启动新进程、中止已有进程等。在 os 模块下与进程管理相关的函数如下。
...
注意：由于os.exec*()函数都是 POSIX  系统的直接映射,因此如果使用该命令来执行 Python 程序,传入的arg0参数没有什么作用。
os._exit(n)用于强制退出Python解释器。 将其放在try块中可以阻止 finally 块的执行。

下面程序示范了在os 模块中与进程管理相关的函数的功能。
"""
import os
...
"""
如果直接运行上面程序,可以看到程序运行后使用 Excel 打开了 abc.xls 文件,也打开了 Notepad ++工具,
还使用 python 命令运行了 os_test.py 文件。但如果将程序中粗体字代码取消注释,
将看到程序运行后只是启动了 cmd 命令行程序,这是因为使用 os.system()函数来运行程序时,新程序所在的进程会替代原有的进程。

在使用 os.execl() 函数运行新进程之后,也会取代原有的进程,因此上面程序将这行代码放在了最后。
"""
