"""
1.  os模块代表了程序所在的操作系统,主要用于获取程序运行所在操作系统的相关信息。
2.  在 Python 的交互式解释器中先导入 os 模块,然后输入os.__all__ 命令(__all__ 变量代表了该模块开放的公开接口),
即可看到该模块所包含的全部属性和函数。开发者同样不需要完全记住这些属性和函数的含义,
在需要用时可参考 https：//docs.python.org/3/library/os.html。
3.  此处仅介绍os 模块中常用的属性和函数。
...
(15)os.urandom(size)：返回适合作为加密使用的、最多由N 个字节组成的bytes对象。
该函数通过操作系统特定的随机性来源返回随机字节,该随机字节通常是不可预测的,因此适用于绝大部分加密场景。
4.  下面程序示范了os 模块的大部分函数的用法。
5.  从上面的输出结果可以看出，在Windows 系统上Python导入依赖模块的操作系统名称为"nt";  
当前系统的登录用户名是"yeeku";当前进程ID 为"3788";当前进程的父进程ID 为"9908";当 前系统上有4个CPU;
