"""
1.  此外,我们还需要考虑一个问题：Python 支持更多的 JSON 所不支持的类型,比如复数、矩阵等,
如果直接使用 dumps() 或 dump() 函数进行转换,程序肯定会出问题。
此时就需要开发者对 JSONEncoder 类进行扩展,通过这种扩展来完成从Python 特殊类型到JSON 类型的转换。
2.  例如,如下程序示范了通过扩展 JSONEncoder 来实现从 Python 复数到 JSON 字符串的转换。
3.  上面程序扩展了 JSONEncoder 类的子类,并重写了它的 default()方法,在方法中判断如果要转换的目标类型是复数(complex),
程序就会进行自定义转换——将复数转换成JSON对象,且该对 象包含"__complex__":'true'属性。
4.  一旦扩展了JSONEncoder 的子类之后,程序有两种方式来使用自定义的子类。
