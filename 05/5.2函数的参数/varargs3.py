"""
1.  Python还可以收集关键字参数,此时Python会将这种关键字参数收集成字典。
为了让Python能收集关键字参数,需要在参数前面添加两个星号。
在这种情况下, 一个函数可同时包含一个支持"普通"参数收集的参数和一个支持关键字参数收集的参数。例如如下代码。
2.  上面程序在调用test()函数时，前面的1、2、3将会传给普通参数x、 y、z; 接下来的两个字符 串将会由books 参数收集成元组；
最后的两个关键字参数将会被收集成字典。运行上面代码，会看 到如下输出结果。
"""
