...
在定义 Python 函数时可定义形参(形式参数的意思),这些形参的值要等到调用时才能确定下来,
由函数的调用者负责为形参传入参数值。简单来说,就是谁调用函数,谁负责传入参数值。

5.2.1 关键字(keyword)参数

Python 函数的参数名不是无意义的,Python 允许在调用函数时通过名字来传入参数值。
...
# 需要说明的是,如果希望在调用函数时混合使用关键字参数和位置参数,则关键字参数必须位于位置参数之后。
...
接下来在调用该函数时,既可使用传统的根据位置参数来调用(如上面程序中第二行粗体字代码所示),
也可根据关键字参数来调用(如上面程序中第三行粗体字代码所示),在使用关键字参数调用时可交换参数的位置
(如上面程序中第四行粗体字代码所示),还可混合使用位置参数和关键字参数(如上面程序中第五行粗体字代码所示)。
