...
Python 的函数也是一种值：所有函数都是 function 对象,这意味着可以把函数本身赋值给变量,
...
"""
从上面代码可以看出,程序依次将 pow()、area()函数赋值给 my_fun 变量,接下来即可通过 my_fun 变量分别调用pow()、area()函数。

其实Python已经内置了计算乘方的方法,因此此处的pow()函数并没有太大的实际意义,只是作为示范使用。

通过对 my_fun 变量赋值不同的函数,可以让 my_fun 在不同的时间指向不同的函数,从而让程序更加灵活。
