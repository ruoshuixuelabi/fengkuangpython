...
conn.close()
"""
上面程序中的粗体字代码调用 execute()方法执行 select 语句查询数据,在该 SQL 语句中同样使用了 %s 作为占位符,
这就是与 SQLite 数据库模块的差别。

该程序直接使用 for 循环来遍历游标所包含的查询数据,这完全是可以的——因为游标本身就是可遍历对象。
运行上面的程序,可以看到如下查询结果。

MySQL  数据库模块的游标对象同样支持fetchone()、fetchmany()、fetchall()方法,
在本书配套代码的code/13/13.3 目录下可以看到一个exec_select2.py程序,该程序使用fetchmany(3))方法每次获取3条记录。
"""
