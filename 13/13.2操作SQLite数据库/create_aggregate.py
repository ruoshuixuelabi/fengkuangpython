...
(3)aggregate_class：指定聚集函数的实现类。该类必须实现 step(self,params...)和 finalize(self)方法,
其中step(方法对于查询所返回的每条记录各执行一次;finalize(self)方法只在最后执行一次,该方法的返回值将作为聚集函数最后的返回值。
...
MinLen 类注册成 min_len 自定义聚集函数,其中 MinLen 类中的 step()方法负责对每个传入的参数进行比较,
选出长度最短的字符串;而 finalize()方法则负责返回长度最短的字符串,该方法的返回值将作为聚集函数的返回值。
第二行粗体字代码在select语句中使用自定义聚集函数,通过该函数就可以选出长度最短的密码
 
运行上面的程序,将可以看到 user_tb 表中长度最短的密码被选出来。
"""