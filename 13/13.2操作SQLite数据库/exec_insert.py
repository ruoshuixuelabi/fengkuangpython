...
使用游标的 execute()方法也可以执行 DML 的 insert、update、delete语句,这样即可对数据库执行插入、修改和删除数据操作。
...
conn.close()
"""
上面程序与 exec_ddl.py程序基本相同,只是程序调用 execute()方法执行的不是 DDL 语句,而是 insert语句,这样程序即可向数据表中插入数据。

由于 Python 的 SQLite 数据库 API 默认是开启了事务的,因此必须调用上面程序中的粗体字代码来提交事务;
