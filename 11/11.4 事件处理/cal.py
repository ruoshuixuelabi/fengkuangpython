"""
读者可能对前面那个徒有其表的计算器感到很失望,下面程序将会为该计算器的按钮绑定事件处理方法,从而使它变成真正可运行的计算器。
"""
...
root.mainloop()
"""
上面程序中第一行粗体字代码为界面上所有按钮的单击事件绑定了处理方法,以便处理程序的计算功能;
第二行粗体字代码则为"="按钮的双击事件绑定了处理方法,当用户双击该按钮时,程序会清空计算结果,重新开始计算。
"""