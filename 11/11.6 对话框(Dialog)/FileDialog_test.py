"""
11.6.4	文件对话框
在 filedialog模块下提供了各种用于生成文件对话框的工具函数,如下所示。这些工具函数有 些返回用户所选择文件的路径,有些直接返回用户所选择文件的输入/输出流。
askopenfile(): 生成打开单个文件的对话框,返回所选择文件的文件流,程序可通过该文件 流来读取文件内容。
askopenfiles():  生成打开多个文件的对话框,返回多个所选择文件的文件流组成的列表, 程序可通过这些文件流来读取文件内容。
askopenfilename():  生成打开单个文件的对话框,返回所选择文件的文件路径。
askopenfilenames():  生成打开多个文件的对话框,返回多个所选择文件的文件路径组成的
元组。
asksaveasfile(): 生成保存文件的对话框,返回所选择文件的文件输出流,程序可通过该文 件输出流向文件写入数据。
asksaveasfilename(): 生成保存文件的对话框,返回所选择文件的文件路径。 askdirectory(): 生成打开目录的对话框。
上面的用于生成打开文件的对话框的工具函数支持如下选项。
defaultextension: 指定默认扩展名。当用户没有输入扩展名时,系统会默认添加该选项指 定的扩展名。
filetypes: 指定在该文件对话框中能查看的文件类型。该选项值是一个序列,可指定多个文 件类型。可以通过“*”指定浏览所有文件。
initialdir: 指定初始打开的目录。
initialfile: 指定所选择的文件。
parent:   指定该对话框的属主窗口。
> title: 指定对话框的标题。
multiple: 指定是否允许多选。
对于打开目录的对话框,还额外支持一个mustexist选项,该选项指定是否只允许打开已存在 的目录。
...
root.mainloop()
"""
从图11.41所示的对话框可以看到,在该对话框中只能浏览文本文件和 Python 源文件,这正 是程序通过 filetypes 选项设置的效果；该对话框默认的打开目录是 G:/根目录,这是程序通过 initialdir选项设置的效果。
提示： -..—..—. ·—..—
通过filedialog模块下的工具函数打开的文件对话框依赖所在的平台,因此在不同
的平台上看到的文件对话框是不同的。

L
..          ..   .                                              
当用户选择指定文件后,可以在控制台看到打印出类似于“< io.TextIOWrapper name='xxx' mode='r'encoding='cp936'>”的输出信息,这就是被打开文件的文件流；如果用户单击“xxx的文  件名”按钮,通过文件对话框选择文件之后,将会在控制台看到只打印出所选择文件的文件路径。
"""