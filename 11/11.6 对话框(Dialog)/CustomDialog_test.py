...
从上面的介绍可以看到,不管是使用 SimpleDialog 还 是 Dialog, 整个对话框的布局都是比较 固定的,开发者只能为其指定title、text等选项,如果希望在对话框中添加其他组件,这就很难实 现了。另外, SimpleDialog 和 Dialog都是模式的。
如果开发者需要使用自定义的对话框,包括定制模式和非模式行为,则可通过继承 Toplevel 来实现。如果打算通过这种方式来实现自定义对话框,有两点要注意。
继   承Toplevel来实现自定义对话框同样需要为对话框指定master。
> 程 序 可 调 用Toplevel的 grab set()方法让该对话框变成模式对话框,否则就是非模式对话框。
下面程序示范了自定义模式和非模式对话框。
"""
from tkinter import *
# 导入ttk
...
root.mainloop()
"""
上面程序定义了一个父类为Toplevel的 MyDialog 类,该类就是一个自定义对话框类,读者以 后完全可以复用这个MyDialog 类。该对话框的主体包含两个方法。
init widgets:  该方法用于初始化对话框的主体界面组件。
init buttons:  该方法用于初始化对话框下方的多个按钮组件。
至于MyDialog 类中的ok click、cancel click,则是程序为对话框按钮所绑定的事件处理方法。 这不是固定的,程序下方有几个按钮,通常就需要为几个按钮绑定事件处理方法。
上面程序中第一行粗体字代码通过 MyDialog 创建了模式对话框；第二行粗体字代码通过


MyDialog 创建了非模式对话框。
运行该程序,单击界面上的“模式对话框”按钮,程序弹出如 图11.37所示的自定义对话框。
在该模式对话框没有关闭的情况下,该程序的主窗口将无法与 用户交互,主窗口无法获得焦点。
如果单击界面上的“非模式对话框”按钮,程序也弹出类似于
...
图11.37所示的自定义对话框,但这个对话框是非模式对话框,即使在该对话框没有关闭的情况下, 用户也依然可以与程序的主窗口交互,主窗口可以获得焦点。
"""