"""
1.  列表的元素相当于变量，因此程序可以对列表的元素赋值，这样即可修改列表的元素。例如如下代码。
2.  下面代码通过索引对列表元素赋值，程序既可使用正数索引，也可使用负数索引，这都没有问题。
3.  此外，程序也可通过 slice 语法对列表其中一部分赋值，在执行这个操作时并不要求新赋值的元素个数与原来的元素个数相等。
这意味着通过这种方式既可为列表增加元素，也可为列表删除元素。例如，如下代码示范了对 列表中间一段赋值(程序清单同上)。
4.  如果对列表中空的slice赋值，就变成了为列表插入元素。例如如下代码。
5.  如果将列表其中一段赋值为空列表，就变成了从列表中删除元素。例如如下代码。
6.  对列表使用slice语法赋值时，不能使用单个值；如果使用字符串赋值，
Python 会自动把字符串当成序列处理，其中每个字符都是一个元素。例如如下代码。
7.  在使用slice语法赋值时，也可指定step参数。但如果指定了step参数，则要求所赋值的列表元素个数与所替换的列表元素个数相等
"""
...
# 在使用slice语法赋值时，也可指定step参数。但如果指定了step参数，则要求所赋值的列表
# 元素个数与所替换的列表元素个数相等。例如如下代码。
