...
append()方法既可接收单个值,也可接收元组、列表等,但该方法只是把元组、列表当成单个元素,
...
"""
在上面代码中,执行完第一行粗体字代码之后,程序向列表中追加了一个元组,
此时被追加的元组只是列表中的一个元素(嵌套元组),接下来输出该列表时将看到如下结果。
['crazyit', 20, -2, 'fkit', (3.4, 5.6)]

在上面代码中,执行完第二行粗体字代码之后,程序再次向列表中追加了一个列表,
此时被追加的列表也只是列表中的一个元素(嵌套列表),接下来输出该列表时将看到如下结果。
['crazyit', 20, -2, 'fkit', (3.4, 5.6), ['a', 'b']]

从上面代码可以看出,为列表追加另一个列表时,Python 会将被追加的列表当成一个整体的元素,而不是追加目标列表中的元素。
如果希望不将被追加的列表当成一个整体,而只是追加列表中的元素,则可使用列表的extend()方法。例如如下代码(程序清单同上)。
