...
(这种参数叫关键字参数,本书第5章会详细介绍)。key 参数用于为每个元素都生成一个比较大小的"键";
reverse参数则用于执行是否需要反转排序——默认是从小到大排序;如果将该参数设为True,将会改为从大到小排序。

如下代码示范了 key 和 reverse参数的用法(程序清单同上)。
# 指定key为len,指定使用len函数对集合元素生成比较的键,也就是按字符串的长度比较大小
b_list.sort(key=len)
print(b_list)  # ['Go', 'Ruby', 'Swift', 'Erlang', 'Kotlin', 'Python']
# 指定反向排序
...
需要指出的是,在 Python 2.x 中,列表的 sort() 方法还可传入一个比较大小的函数,该函数负责比较列表元素的大小。
