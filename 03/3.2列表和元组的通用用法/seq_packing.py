"""
1.  Python 还提供了序列封包(Sequence Packing)和序列解包(Sequence  Unpacking)的功能。
简单来说，Python 允许支持以下两种赋值方式。
(1)程序把多个值赋给一个变量时，Python会自动将多个值封装成元组。这种功能被称为序列封包。
(2)程序允许将序列(元组或列表等)直接赋值给多个变量，
此时序列的各元素会被依次赋值给每个变量(要求序列的元素个数和变量个数相等)。这种功能被称为序列解包。
下面代码示范了序列封包和序列解包的功能。
2.  如果在赋值中同时运用了序列封包和序列解包机制，就可以让赋值运算符支持同时将多个值赋给多个变量。例如如下代码(程序清单同上)。
3.  在序列解包时也可以只解出部分变量，剩下的依然使用列表变量保存。
