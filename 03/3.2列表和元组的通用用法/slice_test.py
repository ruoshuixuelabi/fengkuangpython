"""
3.2.2  子 序 列
1. 与前面介绍的字符串操作类似的是，列表和元组同样也可使用索引获取中间一段，这种用法被称为slice(分片或切片)。
slice 的完整语法格式如下：
[start:end: step]
2. 上面语法中start、end两个索引值都可使用正数或负数，其中负数表示从倒数开始。
该语法表示从start索引的元素开始(包含),到end索引的元素结束(不包含)的所有元素------这和所有编程语言的约定类似。
3. step表示步长，因此step使用负数没有意义。
